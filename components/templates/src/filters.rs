...
pub fn replace_all<S: BuildHasher>(
    ...
    let regex = match args.get("regex") {
        ...
        None => return Err(TeraError::msg("Filter `replace` expected an arg called `regex`")),
    };

    let rep = match args.get("rep") {
        Some(val) => try_get_value!("replace_all", "rep", String, val),
        None => return Err(TeraError::msg("Filter `replace` expected an arg called `rep`")),
    };
    ...
    let regex = match Regex::new(&regex) {
        ...
        Err(err) => {
            return Err(TeraError::msg(format!(
                "Filter `replace`: Invalid regular expression: {}",
                err
            )));
        }
    };
